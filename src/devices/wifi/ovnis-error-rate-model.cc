 ...
 */
#include "ovnis-error-rate-model.h"
#include "ns3/wifi-phy.h"
#include "ns3/log.h"


NS_LOG_COMPONENT_DEFINE ("OvnisErrorRateModel");

namespace ns3 {



  NS_OBJECT_ENSURE_REGISTERED (OvnisErrorRateModel);



TypeId 
OvnisErrorRateModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OvnisErrorRateModel")
    .SetParent<ErrorRateModel> ()
    .AddConstructor<OvnisErrorRateModel> ()
    ;
  return tid;
}
...
OvnisErrorRateModel::OvnisErrorRateModel ()
{}

double 
OvnisErrorRateModel::Log2 (double val) const
{
  return log(val) / log(2.0);
}
double 
OvnisErrorRateModel::GetBpskBer (double snr, uint32_t signalSpread, uint32_t phyRate) const
{
...
double 
OvnisErrorRateModel::GetQamBer (double snr, unsigned int m, uint32_t signalSpread, uint32_t phyRate) const
{
  double EbNo = snr * signalSpread / phyRate;
...
uint32_t
OvnisErrorRateModel::Factorial (uint32_t k) const
{
  uint32_t fact = 1;
  while (k > 0) 
    {
...
double 
OvnisErrorRateModel::Binomial (uint32_t k, double p, uint32_t n) const
{
  double retval = Factorial (n) / (Factorial (k) * Factorial (n-k)) * pow (p, k) * pow (1-p, n-k);
  return retval;
}
double 
OvnisErrorRateModel::CalculatePdOdd (double ber, unsigned int d) const
{
  NS_ASSERT ((d % 2) == 1);
...
double 
OvnisErrorRateModel::CalculatePdEven (double ber, unsigned int d) const
{
  NS_ASSERT ((d % 2) == 0);
...
double 
OvnisErrorRateModel::CalculatePd (double ber, unsigned int d) const
{
  double pd;
  if ((d % 2) == 0) 
    {
...
double
OvnisErrorRateModel::GetFecBpskBer (double snr, double nbits,
...
double
OvnisErrorRateModel::GetFecQamBer (double snr, uint32_t nbits,
...
double 
OvnisErrorRateModel::GetChunkSuccessRate (WifiMode mode, double snr, uint32_t nbits) const
{
