...
#include "applications/CommunityApplication.h"
#include <vector>
#include <math.h>
#include <sstream>
#include <iostream>
...
NS_LOG_COMPONENT_DEFINE("CommunityApplication");
NS_OBJECT_ENSURE_REGISTERED(CommunityApplication);

TypeId CommunityApplication::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::CommunityApplication").SetParent<OvnisApplication>().AddConstructor<CommunityApplication>();
//			AddAttribute("ApplicationParams", "Application parameters", PointerValue(), MakePointerAccessor(&CommunityApplication::applicationParams), MakePointerChecker<Object>());
	return tid;
}
...
void CommunityApplication::InitializeParams() {
 ...
 */
void CommunityApplication::SetNetwork(string networkId) {
	string decisionEdge = "";
...
void CommunityApplication::StartApplication(void) {
	NS_LOG_FUNCTION ("");
	...
	m_socket->SetRecvCallback(MakeCallback(&CommunityApplication::ReceivePacket, this));
...
//	cout << "r " << r << " r2 " << r2 << endl;
//	m_trafficInformationEvent = Simulator::Schedule(Seconds(0), &CommunityApplication::SendTrafficInformation, this);
	m_neighborInformationEvent = Simulator::Schedule(Seconds(r<r2?r:r2), &CommunityApplication::SendNeighborInformation, this);
	m_simulationEvent = Simulator::Schedule(Seconds(r<r2?r2:r), &CommunityApplication::SimulationRun, this);

}

void CommunityApplication::ToggleNeighborDiscovery(bool on) {
	if (on) {
		std::ostringstream oss;
		oss << "/NodeList/" << GetNode()->GetId() << "/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::BeaconingAdhocWifiMac/NeighborLost";
		Config::Connect(oss.str(), MakeCallback(&CommunityApplication::NeighborLost, this));
		std::ostringstream oss2;
		oss2 << "/NodeList/" << GetNode()->GetId() << "/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::BeaconingAdhocWifiMac/NewNeighbor";
		Config::Connect(oss2.str(), MakeCallback(&CommunityApplication::NewNeighborFound, this));
	}
}

void CommunityApplication::StopApplication(void) {
	running = false;
}

void CommunityApplication::DoDispose(void) {
	running = false;
	double now = Simulator::Now().GetSeconds();
	if (m_socket != NULL) {
		m_socket->Close();
	} else {
		NS_LOG_WARN("CommunityApplication found null socket to close in StopApplication");
	}
	OvnisApplication::DoDispose();
}

CommunityApplication::MacAddrMap CommunityApplication::getNeighborList() {
...
void CommunityApplication::SimulationRun(void) {
	try {
		if (running == true) {
			getNeighborList();
			double now = Simulator::Now().GetSeconds();
			...
			m_simulationEvent = Simulator::Schedule(Seconds(SIMULATION_STEP_INTERVAL), &CommunityApplication::SimulationRun, this);
		}
	}
 ...
 */
void CommunityApplication::OnEdgeChanged(double now, string currentEdgeId) {
...
map<string, double> CommunityApplication::EstimateTravelCostBasedOnCentralised(double now, string currentEdgeId) {
...
map<string, double> CommunityApplication::EstimateTravelCostBasedOnVanets(double now, string currentEdgeId, string costFunction) {
	Vector position = mobilityModel->GetPosition();
...
string CommunityApplication::ChooseRoute(double now, string currentEdgeId, map<string, double> routeCost, string routingStrategy, double cheatersRatio) {
 ...
 */
void CommunityApplication::OnReporting(double now, string currentEdgeId) {
...
map<string, double> CommunityApplication::AnalyseRouteCorrelation() {
	string currentEdgeId = vehicle.getItinerary().getCurrentEdge().getId();
	string endEdgeId = vehicle.getDestinationEdgeId();
...
void CommunityApplication::CalculateError(string currentEdge) {
...
void CommunityApplication::SendNeighborInformation(void) {
	if (running == true) {
		Vector position = mobilityModel->GetPosition();
...
//		}
		...
		SendPacket(p);
		m_neighborInformationEvent = Simulator::Schedule(Seconds(NEIGHBOR_INFORMATION_SENDING_INTERVAL), &CommunityApplication::SendNeighborInformation, this);
	}
}

void CommunityApplication::SendTrafficInformation(void) {
	if (running == true) {
		Vector position = mobilityModel->GetPosition();
...
//			}
		...
		m_trafficInformationEvent = Simulator::Schedule(Seconds(TRAFFIC_INFORMATION_SENDING_INTERVAL), &CommunityApplication::SendTrafficInformation, this);
	}
}

void CommunityApplication::SendPacket(Ptr<Packet> packet) {
...
void CommunityApplication::ReceivePacket(Ptr<Socket> socket) {
 ...
 */
void CommunityApplication::NeighborLost(std::string context, Ptr<const Packet> packet, Mac48Address addr){
	...
	Ptr<CommunityApplication> myApp = DynamicCast<CommunityApplication>(app);
 ...
 * Neighborhood discovery
 */
void CommunityApplication::NewNeighborFound(std::string context, Ptr<const Packet> packet, Mac48Address addr, double rxPwDbm) {
	// Find the nodeId that has been called
	size_t aux =context.find("/",10);
	std::string strIndex = context.substr(10,aux-10);
	// Convert the index to integer
